...
5. **View your release** at: `https://github.com/yourusername/mtgpricer_v2/releases`
...
# Executable will be in: dist/mtgpricer
```
...
# Executable will be in: dist\mtgpricer.exe
```
...
### Windows
Double-click `dist\mtgpricer.exe` or run from cmd:
```cmd
dist\mtgpricer.exe
```
...
- **Linux users**: Download `mtgpricer-linux`, make it executable (`chmod +x`), and run it
- **Windows users**: Download `mtgpricer-windows.exe` and double-click to run
